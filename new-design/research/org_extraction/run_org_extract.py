...
def anchor_check(obj, text):
    """锚点校验(NFKC 归一,与后处理一致)。返回 bad 列表。"""
    n=PP._nfkc; T=n(text)
    bad=[]
    for o in obj.get("organizations",[]):
        for m in o.get("mentions",[]):
            if m and n(m) not in T: bad.append(("mention",o.get("name"),m))
    for mem in obj.get("memberships",[]):
        if mem.get("source")=="explicit":
            at=mem.get("anchor_text","")
            if not at or n(at) not in T: bad.append(("anchor",mem.get("character_name"),at))
    return bad
...
def run_ch(ch, runs=3):
    ...
    for i in range(runs):
        raw=call(text)
        try: obj_raw=json.loads(raw)
        except Exception as e:
            print(f"  [{ch} run{i}] JSON parse FAIL: {e}"); results.append(None); continue
        obj, dropped = PP.clean_organizations(obj_raw, text)
        bad=anchor_check(obj,text)   # 后处理后应为 0
        ...
        print(f"  [{ch} run{i}] orgs={orgs} | mem={len(mems)}(expl={sum(1 for m in mems if m[2]=='explicit')}) | anchor_bad={len(bad)} | dropped={len(dropped)}")
        if dropped: print(f"      dropped: {dropped[:5]}")
        if bad: print(f"      BAD anchors(残留): {bad[:3]}")
