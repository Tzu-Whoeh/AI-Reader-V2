...
- 历史运行参考(**某次历史生产运行实测,非回归门槛/不变量**):168 人物 / 2425 关系 /
  0 id 越界 / 正确合并 / 无超级簇。引用时勿当作验收基线,代码或语料变动后数字会变。
- 冷启动验收:首跑空缓存时,跨章聚合的人物清洗/复核会对全部新词/新对发起模型判定,
  与逐章分析争抢同一 GPU,可能拖垮管线。应对是**暖缓存策略**(填满 `output/<slug>/.review_cache`
  后再正式跑),详见部署规格 §7。**注:仓库当前无 warmcache 脚本**,暖缓存是运行策略而非现成命令;
  若需一键化须单独提 PR。
