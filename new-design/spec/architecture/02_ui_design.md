...
⚠️ **`views/Upload.jsx` 是死代码**:导出存在但**无任何文件 import**;上传 UI 实际内联在
`Library.jsx`(`uploadFile`+`startAnalyze`)。建议后续清理或重新接线。
> 验证:`grep -rn "Upload" app/web/src/*.jsx app/web/src/**/*.jsx | grep import` → 无结果。
...
- **渲染**:纯 SVG `<line>`/`<circle>`/`<text>`;节点半径按类型(character 9 / organization 8 / event 5 / 其余 6);
  自环边过滤(`e.from!==e.to`)。
- **Filters**:三类型(character/item/location)显隐复选;注意图谱实际可含 event/organization 节点,
  但 Filters 只暴露三类(event/org 恒显)。依据:App.jsx `show` 初值仅三键 + Filters.jsx。
...
- **Upload.jsx 死代码**:未接线,与 Library 内联上传重复,应清理或复用(§2)。
- **Filters 只覆盖三类型**:event/organization 节点在图谱恒显、无显隐开关(§5.1)。
- **event/org 颜色硬编码**:`#9a7db8`/`#4a8fb8` 在 GraphPane.jsx 写死,未纳入 `theme.css` 变量,
  改色需改两处(GraphPane 边色 + 节点色),易漏(§6)。
- **无 URL 路由**:视图状态不进 URL,刷新回默认 graph、不可深链/分享具体视图(§1)。
- **图布局非确定**:`computeLayout` 含 `Math.random()` 抖动解重叠,同图多次渲染坐标可能不同(§5.1)。
