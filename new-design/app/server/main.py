...
def register_readonly():
    ...
    def novels():
        ...
        for n in nv:
            ...
            if m:
                for k in ("tags", "cover", "rules_selected", "function_tag_catalog", "action_tag_catalog", "partial_reason", "error_count", "first_error"):
...
def _run_analysis(slug):
    ...
    try:
        ...
        atcat = meta0.get("action_tag_catalog")    # 单书动作标签候选清单;None/空 → pipeline 回退内置默认
        ...
        pipeline.run(novel_input(slug), out_dir=out, presplit=True, progress_cb=cb, should_continue=control, function_catalog=ftcat, action_catalog=atcat)
...
def register_library_admin():
    ...
    def novel_meta_update(slug):
        ...
        if "action_tag_catalog" in body:
            v = body["action_tag_catalog"]
            # 仅接受字符串列表;去空白/去重/限长(单词级,与 sanitize_action_tags 一致的精神)
            if isinstance(v, list):
                seen=set(); clean=[]
                for t in v:
                    if not isinstance(t, str): continue
                    ...
                    if t and 2<=len(t)<=5 and t not in seen:
                        seen.add(t); clean.append(t)
                meta["action_tag_catalog"] = clean
            else:
                meta["action_tag_catalog"] = None
        write_meta(slug, meta)
